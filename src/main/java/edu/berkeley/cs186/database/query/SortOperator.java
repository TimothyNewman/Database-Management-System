...
public class SortOperator {
    ...
    public Run sortRun(Run run) {
        ...
        run.iterator().forEachRemaining(listOfRecords::add);
        listOfRecords.sort(comparator);
        tempRun.addRecords(listOfRecords);
        return tempRun;
    }
    ...
    public Run mergeSortedRuns(List<Run> runs) {
        // TODO(proj3_part1): implement
        Run tempRun = createRun();
        List<Iterator<Record>> itList = new ArrayList<>();
        Queue<Pair<Record, Integer>> priorityQueue = new PriorityQueue<>(new RecordPairComparator());
        int index = 0;
        for (Run curr : runs) {
            itList.add(index, curr.iterator());
            if (itList.get(index).hasNext()) {
                priorityQueue.add(new Pair<>(itList.get(index).next(), index));
            }
            index += 1;
        }
        while (!priorityQueue.isEmpty()) {
            Pair<Record, Integer> p = priorityQueue.poll();
            tempRun.addRecord(p.getFirst().getValues());
            if (itList.get(p.getSecond()).hasNext()) {
                priorityQueue.add(new Pair<>(itList.get(p.getSecond()).next(), p.getSecond()));
            }
        }
    ...
    public List<Run> mergePass(List<Run> runs) {
        ...
        while (index < runs.size()) {
            List<Run> r = runs.subList(index, index + numBuffers-1);
            sorted.add(mergeSortedRuns(r));
            index += numBuffers - 1;
        }
        return sorted;
    }
    ...
    public String sort() {
        ...
        BacktrackingIterator<Page> pageIt = transaction.getPageIterator(tableName);

        while (pageIt.hasNext()) {
            BacktrackingIterator<Record> blockIt = transaction.getBlockIterator(tableName, pageIt, numBuffers);
            Run tempRun = createRunFromIterator(blockIt);
            sort.add(sortRun(tempRun));
        }
        while (1 < sort.size()) {sort = mergePass(sort);}

        return sort.get(0).tableName();
    }
