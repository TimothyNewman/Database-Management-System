...
public class QueryPlan {
    ...
    public Iterator<Record> execute() {
        ...
        Map<Set, QueryOperator> first_mapping = new HashMap<>();
        Set<String> names = new HashSet<>();
        names.add(startTableName);
        names.addAll(joinTableNames);
        for (String n : names) {
            QueryOperator query_Op = minCostSingleAccess(n);
            Set<String> key = new HashSet<>();
            key.add(n);
            first_mapping.put(key, query_Op);
        }
        int i = 1;
        Map<Set, QueryOperator> next_mapping = first_mapping;
        while (i<names.size()) {
            next_mapping = minCostJoins(next_mapping, first_mapping);
            i++;
        }
        finalOperator = minCostOperator(next_mapping);
        addGroupBy();
        addProjects();
    ...
    QueryOperator minCostSingleAccess(String table) {
        ...
        int minOp_cost = minOp.estimateIOCost();
        List<Integer> eligible_columns = getEligibleIndexColumns(table);
        int except = Integer.MIN_VALUE;
        int e = 0;
        while (e<eligible_columns.size()) {
            QueryOperator queryOp = new IndexScanOperator(this.transaction, table, selectColumnNames.get(e),
                                                        selectOperators.get(e), selectDataBoxes.get(e));
            int queryOp_cost = queryOp.estimateIOCost();
            if (queryOp_cost < minOp_cost) {
                minOp = queryOp;
                except = e;
            }
            e++;
    ...
    Map<Set, QueryOperator> minCostJoins(Map<Set, QueryOperator> prevMap,
        ...
        for (Set tables : prevMap.keySet()) {
            for (String name : joinTableNames) {
                String[] L = getJoinLeftColumnNameByIndex(joinTableNames.indexOf(name));
                String[] R = getJoinRightColumnNameByIndex(joinTableNames.indexOf(name));
                QueryOperator current_Qop = prevMap.get(tables);
                QueryOperator join_Qop;
                Set<String> keys = new HashSet<>();
                if (tables.contains(L[0]) && !tables.contains(R[0])) {
                    keys.add(R[0]);
                    QueryOperator compare_Qop = pass1Map.get(keys);
                    join_Qop = minCostJoinType(current_Qop, compare_Qop, L[1], R[1]);
                } else if (!tables.contains(L[0]) && tables.contains(R[0])) {
                    keys.add(L[0]);
                    QueryOperator compare_Qop = pass1Map.get(keys);
                    join_Qop = minCostJoinType(current_Qop, compare_Qop, R[1], L[1]);
                } else {continue;}
                Set<String> results = new HashSet<>();
                results.addAll(tables);
                results.addAll(keys);
                if (!map.containsKey(results)) {
                    map.put(results, join_Qop);
                } else {
                    if (join_Qop.estimateIOCost()< map.get(results).estimateIOCost()) {
                        map.put(results, join_Qop);
                    }
                }
            }
