...
public class GraceHashJoin {
    ...
    private List<Record> buildAndProbe(HashPartition partition) {
        ...
        while (buildRecords.hasNext()) {
            Record b_Record = buildRecords.next();
            DataBox buildJoinValue = b_Record.getValues().get(buildColumnIndex);

            if (!hashTable.containsKey(buildJoinValue)) {
                hashTable.put(buildJoinValue, new ArrayList<>());
            }
            hashTable.get(buildJoinValue).add(b_Record);
        }

        // Probing stage
        while (probeRecords.hasNext()) {
            Record p_Record = probeRecords.next();
            DataBox probeJoinValue = p_Record.getValues().get(probeColumnIndex);

            if (hashTable.containsKey(probeJoinValue)) {
                for (Record r : hashTable.get(probeJoinValue)) {
                    if (probeFirst) {
                        Record joinedRecord = joinRecords(p_Record, r);
                        joinedRecords.add(joinedRecord);
                    }
                    if (!probeFirst) {
                        Record joinedRecord = joinRecords(r, p_Record);
                        joinedRecords.add(joinedRecord);
                    }
                }
            }
