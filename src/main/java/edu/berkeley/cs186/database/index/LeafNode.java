...
class LeafNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        // TODO(proj2): implement
        if (keys.contains(key)) {throw new BPlusTreeException("B+ trees do not support duplicate entries with the same key.");}
        ...
        return put_help(rightKeys, rightIds);
    }
    public Optional<Pair<DataBox, Long>> put_help(List<DataBox> k, List<RecordId> ids) {
        if (metadata.getOrder() < keys.size()) {
            k.add(keys.remove(metadata.getOrder()));
            ids.add(rids.remove(metadata.getOrder()));
            return put_help(k, ids);
        } else {
            LeafNode splitNode = new LeafNode(metadata, bufferManager, k, ids, rightSibling, treeContext);
            rightSibling = Optional.of(splitNode.getPage().getPageNum());
            sync();
            return Optional.of(new Pair<>(splitNode.keys.get(0), splitNode.getPage().getPageNum()));
        }
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data, float fillFactor) {
        // TODO(proj2): implement
        Optional<Pair<DataBox, Long>> optionalPair = Optional.empty();
        int fillLimit = (int)Math.ceil(fillFactor*metadata.getOrder()*2);
        if ((fillLimit>=keys.size()) && data.hasNext()) {bulk_help(fillLimit, data);}
        if (fillLimit<keys.size()) {
            List<DataBox> k = new ArrayList<>();
            List<RecordId> r = new ArrayList<>();
            int i = keys.size()-1;
            k.add(keys.remove(i));
            r.add(rids.remove(i));
            LeafNode n = new LeafNode(metadata, bufferManager, k, r, rightSibling, treeContext);
            Pair<DataBox, Long> addPair = new Pair<>(k.get(0), n.getPage().getPageNum());
            optionalPair = Optional.of(addPair);
        }
        sync();
        return optionalPair;
    }
    public void bulk_help(int fill, Iterator<Pair<DataBox, RecordId>> data) {
        Pair<DataBox, RecordId> next = data.next();
        keys.add(InnerNode.numLessThan(next.getFirst(), keys), next.getFirst());
        rids.add(InnerNode.numLessThan(next.getFirst(), keys), next.getSecond());
        if ((fill>=keys.size()) && data.hasNext()) {
            bulk_help(fill, data);
        }
    }
    ...
    public static LeafNode fromBytes(BPlusTreeMetadata metadata, BufferManager bufferManager,
        ...
        long rightSibsPageID = buf.getLong();
        int numberOfPairs = buf.getInt();
        Optional<Long> rightSibling = rightSibsPageID == -1 ? Optional.empty() : Optional.of(rightSibsPageID);
