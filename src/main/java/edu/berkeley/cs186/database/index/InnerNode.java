...
class InnerNode extends BPlusNode {
    ...
    public LeafNode get(DataBox key) {
        ...
        return leaf_get_helper(key, getChild(numLessThanEqual(key, keys)));
    }
    public LeafNode leaf_get_helper(DataBox key, BPlusNode node) {
        if (node instanceof InnerNode) {
            return leaf_get_helper(key, node.get(key));
        }
        return node.get(key);
    }
    ...
    public LeafNode getLeftmostLeaf() {
        ...
        // TODO(proj2): implement
        return leaf_getleft_helper(getChild(0));
    }
    public LeafNode leaf_getleft_helper(BPlusNode leftChild) {
        if (leftChild instanceof InnerNode) {
            return leaf_getleft_helper(leftChild.getLeftmostLeaf());
        }
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        Optional<Pair<DataBox, Long>> optionalPair = child.put(key, rid);
        if (optionalPair.isPresent()) {
            int i = InnerNode.numLessThanEqual(optionalPair.get().getFirst(), keys);
            keys.add(i, optionalPair.get().getFirst());
            children.add(i+1, optionalPair.get().getSecond());
            if ((metadata.getOrder()*2)>=keys.size()) {optionalPair = Optional.empty();}
            else {
                List<DataBox> k = new ArrayList<>();
                List<Long> c = new ArrayList<>();
                c.add(children.remove(metadata.getOrder()+1));
                optionalPair = put_else_helper(k, c);
            }
        }
        sync();
        return optionalPair;
    }
    public Optional<Pair<DataBox, Long>> put_else_helper(List<DataBox> listOfKeys, List<Long> listOfChildren) {
        if (metadata.getOrder()+1 < keys.size()) {
            listOfKeys.add(keys.remove(metadata.getOrder()+1));
            listOfChildren.add(children.remove(metadata.getOrder()+1));
            return put_else_helper(listOfKeys, listOfChildren);
        } else {
            InnerNode newRIN = new InnerNode(metadata, bufferManager, listOfKeys, listOfChildren, treeContext);
            return Optional.of(new Pair<>(keys.remove(metadata.getOrder()), newRIN.getPage().getPageNum()));
        }
    }
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        if ((metadata.getOrder()*2>=keys.size()) && data.hasNext()) {
            optionalPair = bulkLoad_less_helper(data, fillFactor);
        }
        if (metadata.getOrder()*2<keys.size()) {
            List<DataBox> k = new ArrayList<>();
            List<Long> c = new ArrayList<>();
            k.add(keys.remove(metadata.getOrder()+1));
            optionalPair = bulkLoad_more_helper(k, c);
        }
        else {optionalPair = Optional.empty();}
        sync();
        return optionalPair;
    }
    public Optional<Pair<DataBox, Long>> bulkLoad_less_helper(Iterator<Pair<DataBox, RecordId>> data, float fillFactor) {
        BPlusNode right = getChild(children.size()-1);
        Optional<Pair<DataBox, Long>> optionalPair = right.bulkLoad(data, fillFactor);
        if (optionalPair.isPresent()) {
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad_more_helper(List<DataBox> listOfKeys, List<Long> listOfChildren) {
        if (metadata.getOrder()+1<keys.size()) {
            listOfKeys.add(keys.remove(metadata.getOrder()+1));
            listOfChildren.add(children.remove(metadata.getOrder()+1));
            bulkLoad_more_helper(listOfKeys, listOfChildren);
        }
        InnerNode inNode = new InnerNode(metadata, bufferManager, listOfKeys, listOfChildren, treeContext);
        return Optional.of(new Pair<>(keys.remove(metadata.getOrder()), inNode.getPage().getPageNum()));
    }
